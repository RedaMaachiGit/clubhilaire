<?php
  ...
  $vendeur = unserialize(urldecode($_SESSION['vendeur']));

  $i=0;
  $j=0;
  $paiement = false;
  $prePaiement = false;
  $prePaiementTotal = false;
  $prixMarge = 0;
  $prixPrepMarge = 0;
  foreach($lots as $lot){
    $prixLot = $lot->getPrix();
    ...
    if($statut === "Restitue" || $statut === "Paye remis" || $statut === "Invalide"){
      unset($lots[$i]);
    } else if ($statut === "Vendu"){
      $paiement = true;
      $prixMarge = $prixMarge + $lot->getPrixMoinsMarge();
    } else if ($statut === "Prepaye"){
      $prixPrepMarge = $prixPrepMarge + $lot->getPrixMoinsMarge();
      $prePaiement = true;
      $j++;
    }
    $i++;
...
<head>
  ...
  <title>Restituer des lots</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Restitution d'un ensemble de lots
        <small>Vous êtes sur le point de restituer un ensemble de lots</small>
      </h1>
      <ol class="breadcrumb">
      ...
      <?php if($prePaiement){ ?>
        <div class="col-md-12">
          <div class="alert alert-warning fade in">
              ...
              <strong>ATTENTION!</strong> Un paiement a déjà été préparé pour ce vendeur. Pensez à lui remettre.
          </div>
        </div>
      <?php } ?>
      <?php if($paiement){ ?>
        <div class="col-md-12">
          ...
          <div class="col-md-12">
            <div class="box box-info">
              <form id="paiementForm" class="form-horizontal" method="POST" action="../controller/controllerRestitutionMail.php" class="form-horizontal">
                <div class="box-body">
                ...
                <div class="box-footer">
                  <input class="form-control input-lg" name="mail" id="mail" type="hidden" value=<?php echo $vendeur->getEmail(); ?> >
        ...
        <?php } ?>
        <?php if(!$paiement && $prePaiement){ ?>
          <div class="col-sm-12">
            <div class="box box-info">
              <form id="paiementForm" class="form-horizontal" method="POST" action="../controller/controllerRestitutionMail.php" class="form-horizontal">
                <div class="box-body">
                  <lable for="numeroLot"> Cliquez sur "Remettre paiement" une fois le paiement préalablement préapré remis au vendeur, pour rappel la somme totale est de: <u><b><?php echo $prixPrepMarge ?></b></u>. </label>
                </div>
                <div class="box-footer">
                    <input class="form-control input-lg" name="mail" id="mail" type="hidden" value=<?php echo $vendeur->getEmail(); ?> >
                    <input class="form-control input-lg" name="paiement" id="paiement" type="hidden" value=<?php echo $prixMarge ?>>
                  <button type="submit" value="Submit" class="btn btn-info center-block">Remettre paiement</button>
                </div>
              </form>
            </div>
          </div>
            ...
            <div class="row">
              <div class="col-sm-12">
                <table id="example1" class="table table-hover">
                  ...
                  <tbody>
                    <?php foreach ($articles as $article) { // foreach ($shop as $row) : ?>
                      <tr>
